...
const executeTraining = () => {
    console.log("loading training set: this may take a few minutes")

    //inital training data 
    classifier.addDocument('3½ ounces (100 grams) dried shredded pollock', 'ingredient');
    classifier.addDocument('2 cloves garlic, minced', 'ingredient');
    classifier.addDocument('½ cup gochujang (Korean hot pepper paste)', 'ingredient');
    classifier.addDocument('½ cup rice syrup (or corn syrup)', 'ingredient');
    classifier.addDocument('2 tablespoons sesame oil', 'ingredient');
    classifier.addDocument('2 tablespoons vegetable oil ', 'ingredient');
    classifier.addDocument('3 1/4 cups (455 g) all-purpose flour', 'ingredient');
    classifier.addDocument('3/4 cup (100 g) nonfat dry milk powder', 'ingredient');
    classifier.addDocument('1 1/4 cup (285 g or 2 1/2 sticks) unsalted butter at room temperature', 'ingredient');
    classifier.addDocument('1 1/4 cup (275 g) dark brown sugar', 'ingredient');
    classifier.addDocument('1 teaspoon kosher salt', 'ingredient');
    classifier.addDocument('2 large eggs', 'ingredient');
    ...
    // limit 20 recipes for training per current performance
    // for (let i = 0; i < 10; i++) {
    ...
    classifier.save('classifier.json', function (err, classifier) {
        ...
        console.log('trained and saved classifier')
        return;
    });
};
