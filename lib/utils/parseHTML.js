...
const parseHTML = async (html, url) => {
  ...
  if (attemptedSchemaScrape !== -1) {
    ...
    const directionSet = await extractSetWithClassifier(...directionSection, determineDirectionSet);

    extractedData = attemptedPropScrape;

    if (attemptedPropScrape.directions.length < directionSet.length && directionSet.length > 2) {
      extractedData.directions = directionSet;
    }

  } else {
    // no reliable schema, proceed to manually parse
    ...
    const ingredientSet = await extractSetWithClassifier(...ingredientSection, determineIngredientSet);
...
const extractSetWithClassifier = async (section, indexOf = 1, classifier) => {
  try {
