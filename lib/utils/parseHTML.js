...
const parseHTML = async (html) => {
    ...
    if(findScriptSchema.length) {
        // appSchema.push(Object.keys(findScriptSchema).filter((el) => {
        //     if(findScriptSchema[el]) {
        //         findScriptSchema[el].attribs.type === "application/ld+json";
        //     }
        // }));
        const scriptString = findScriptSchema.html().trim();
        console.log(JSON.parse(scriptString))
        // console.log(findScriptSchema[0])
        console.log(appSchema);

    }
    // console.log(findSchema.html)
